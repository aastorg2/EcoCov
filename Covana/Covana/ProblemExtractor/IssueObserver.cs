...
//            e.
        ...
        private void Log_UninstrumentedMethodHandler(UninstrumentedMethodEventArgs e)
        {
            ...
            if (database.extToPUTs.ContainsKey(method.Method.FullName))
            {
                //File.WriteAllText(@"C:\tempSeqex\debug.txt", "enter");
                if (!database.extToPUTs[method.Method.FullName].Contains(currentPUT.FullName))
                {
                    database.extToPUTs[method.Method.FullName].Add(currentPUT.FullName);
                }
            }
            else
            {
                //File.WriteAllText(@"C:\tempSeqex\debug.txt", "enter1");
                //File.WriteAllText(@"C:\tempSeqex\debug.txt", "currentPUT is null: "+(currentPUT == null));
                List<string> list = new List<string>();
                list.Add(currentPUT.FullName);
                database.extToPUTs.Add(method.Method.FullName, list);
            }
            //StringBuilder sb = new StringBuilder();
            //sb.AppendLine("extToPuts: " + database.extToPUTs.Count);
            //File.WriteAllText(@"C:\tempSeqex\debug.txt", sb.ToString());
        }
