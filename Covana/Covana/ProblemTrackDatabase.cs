...
namespace Covana
{
    ...
    public class ProblemTrackDatabase : PexComponentBase, IService
    {
        ...
        public Method CurrentPUTMethod { get; set; }
        ...
        public Dictionary<MethodDefinition, Dictionary<MethodDefinition, HashSet<BranchCoverageDetail>>> PUTBranchCoverageDetails
            = new Dictionary<MethodDefinition, Dictionary<MethodDefinition, HashSet<BranchCoverageDetail>>>();
        ...
        public List<PUT> PUTs = new List<PUT>();
        ...
        public StringBuilder PUTCoverageSummaryLog = new StringBuilder("CovSummary: \n");
        ...
        public Dictionary<string, List<string>> extToPUTs = new Dictionary<string, List<string>>();
        public Dictionary<string, List<string>> ecpToPUTs = new Dictionary<string, List<string>>();
        ...
        public void AfterExecution()
        {
            try
            {
                ...
                var problemdumper = new CovanaProblemDumper();
                problemdumper._candidateObjectCreationProblems = CandidateObjectCreationProblems;
                problemdumper._nonCoveredBranchInfo = _nonCoveredBranchInfo;
                problemdumper.resultTrackingInfo = resultTrackingInfo;
                problemdumper.uninstrumentedMethodsFoundInObj = uninstrumentedMethodsFoundInObj;
                problemdumper.ExceptionExternalMethods = ExceptionExternalMethods;
                problemdumper.ExternalMethods = this.ExternalMethods;
                problemdumper.ExternalMethodInBranch = this.ExternalMethodInBranch;
                problemdumper.extToPUTs = this.extToPUTs;
                problemdumper.ecpToPUTs = this.ecpToPUTs;
                problemdumper.DumpProblems(AssemblyUnderTest);

                XmlSerializer serializer = new XmlSerializer(typeof(List<PUT>));
                TextWriter xmlwriter = new StreamWriter(@"c:\tempseqex\puts.xml");
                serializer.Serialize(xmlwriter, PUTs);
                xmlwriter.Close();

                foreach (var pair in PUTBranchCoverageDetails)
                {
                    var put = pair.Key;
                    var putcoverage = pair.Value;
                    var methodname = "[" + put.ShortName + "]";
                    var namespacename = "[]";
                    var typename = "[]";
                    TypeDefinition td;
                    if (put.TryGetDeclaringType(out td))
                    {
                        typename = "[" + td.ShortName + "]";
                        namespacename = "[" + td.Namespace + "]";
                    }
                    var putcoverageDetailsFileName = xmlFileDirectory + namespacename + "." + typename + "." + methodname + ".coverageDetails" + ".xml";
                    DumpXml(putcoverage, putcoverageDetailsFileName);
                }
            }
            catch (Exception e)
            {
        ...
        private void DumpCoverageDetails()
        {
            ...
            //DumpXml(BranchCoverageDetails, coverageDetailsFileName + ".xml");
            DumpCoverage(BranchCoverageDetails);
        }


        private void DumpCoverage(Dictionary<MethodDefinition, HashSet<BranchCoverageDetail>> branchCoverageDetails)
        {
            var covlist = new List<BranchCoverageDetail>();
            foreach (KeyValuePair<MethodDefinition, HashSet<BranchCoverageDetail>> pair in BranchCoverageDetails)
            {
                foreach (BranchCoverageDetail coverageDetail in pair.Value)
                {
                    covlist.Add(coverageDetail);
                }
            }

            XmlSerializer covSerializer = new XmlSerializer(typeof(List<BranchCoverageDetail>));
            XmlTextWriter ocpwriter = new XmlTextWriter(new FileStream(@"c:\tempseqex\covdetails.xml", FileMode.Create), Encoding.UTF8);
            ocpwriter.Formatting = Formatting.Indented;
            covSerializer.Serialize(ocpwriter, covlist);
            ocpwriter.Close();
        }
        ...
        private void DumpUncoveredBranches()
        {
            ...
            File.WriteAllText(@"c:\tempseqex\putcovsummary.txt", PUTCoverageSummaryLog.ToString());
